...
final class RedisEnhancer {
    ...
    static void enhance(MetaClass mc, RedisProvider provider = RedisPoolHolder.instance) {
        ...
        mc.withRedis = {Closure closure ->
            provider.withRedis('default', closure)
        }
        mc.withRedis << {String datasourceName, Closure closure ->
            provider.withRedis(datasourceName, closure)
        }
        mc.withRedis << {CallableWithArgs callable ->
            provider.withRedis('default', callable)
        }
        mc.withRedis << {String datasourceName, CallableWithArgs callable ->
            provider.withRedis(datasourceName, callable)
        }
    }
}
